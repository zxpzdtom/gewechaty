...
import { compareMemberLists, getAttributesFromXML, getLocalIPAddress } from '@/utils/index.js';
    ...
    .post(option.route, async (ctx) => {
      try {
        ...
        if (body.Appid && body.TypeName === 'AddMsg') {
          ...
          // 发送消息
          const type = msg.type();
          if (type === MessageType.RoomInvitation) {
            // 群邀请
            const obj = Message.getXmlToJson(msg.text());
            obj.fromId = msg.fromId;
            bot.emit('room-invite', new RoomInvitation(obj));
          } else if (type === MessageType.AddFriend) {
            // 好友请求
            const obj = getAttributesFromXML(msg.text());
            bot.emit('friendship', new Friendship(obj));
          } else if (type === MessageType.Revoke) {
            // 消息撤回
            bot.emit('revoke', msg);
          } else {
            bot.emit('message', msg);
          }
        } else if (body && body.TypeName === 'ModContacts') {
          // 好友消息， 群信息变更
          // 消息hanlder
          const id = body.Data?.UserName?.string || '';
          // biome-ignore lint/suspicious/noDoubleEquals: <explanation>
          if (id.endsWith('@chatroom') && body.Data?.ImgFlag != 1) {
            // 群消息
            ...
            if (body.Data.NickName.string !== oldInfo.nickName) {
              // 群名称变动
